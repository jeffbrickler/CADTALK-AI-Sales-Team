> **CADTALK leadership reference.** Read by `/ct-cro` (leader-facing advisory). Universal B2B-SaaS revenue frameworks — pair with CADTALK rep skills: `/ct-commit` (weighted forecast), `/ct-report` (pipeline view), `/ct-qualify` Coach Mode (deal health). Advisory only: any Pipedrive read goes through the sales-crm contract (`agents/sales-crm.md`); this module never writes the CRM. Written output follows `references/cadtalk-voice-reference.md`.
